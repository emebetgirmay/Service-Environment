...
#
#   SG-1  Internet -> ALB :80            — inline in the alb module (self-contained)
#   SG-2  ALB -> Service A :3001         — below
#   SG-3  Service A -> Service B :3002   — below
#   SG-4  Service B -> Service C :3003   — below
#   SG-5  Internet -> A/B/C directly     — implicit deny: no rule written, ever
#   SG-6  Service A -> Service C         — implicit deny: no rule written, ever
#   SG-7  Service C -> Service A callback — NOT implemented. See Predicted
#         Edge #1 / Scar 1 in the Gate 1 doc. The app still calls it; the
#         call fails closed until this is explicitly resolved.

# SG-2: ALB -> Service A :3001
resource "aws_vpc_security_group_ingress_rule" "alb_to_service_a" {
  security_group_id            = module.service_a.security_group_id
  description                  = "SG-2: ALB -> Service A :3001"
